...
struct AutobuyerView: View {
    ...
    var body: some View {
        LazyVGrid(columns: columns) {
            ForEach(autobuyers.filter {$0.state.unlocked.wrappedValue}) {autobuyer in
                VStack{
                    if let adautobuyer = autobuyer.wrappedValue as? AMDimensionAutobuyer {
                        ...
                        HStack {
                            Button(action: {adautobuyer.toggleEnabled()}) {
                                adautobuyer.state.enabled ? Label("Enabled", systemImage: "checkmark.circle.fill").labelStyle(.iconOnly) : Label("Disabled", systemImage: "checkmark.circle").labelStyle(.iconOnly)
                            }.padding()
